  ...
  window.onload = function () {
    ...
    class Main {
      constructor() {
        ...
        this.form = document.getElementById('mosaicForm');
    ...
    // Instantiate a new object of Main to attach the events and run program
    let main = new Main();
  };
}());

// If color is same don't do network request. Caching the request
// Auto optimize for sending too many requests, use some kind of convoluted server etc..
// class, Promise, Worker and fat arrow function are supported now in Chrome
// Could utilize caching with promises
// Other solution where render after we resolve network request

// loadSVG(mosaicEl, color);

// function loadSVG(mosaicEl, color) {
...
//       });
//   } else {
//     tile.innerHTML = cache[color];
//   }
// }

// Promise.all(ps)
//   .then(function (rs) {
//     rs.forEach(function (responses, idx) {
//       let tiles = tileElements[idx];
//       responses.forEach(function (response, i) {
//         tiles[i].innerHTML = response;
//       });
//     });
//   });